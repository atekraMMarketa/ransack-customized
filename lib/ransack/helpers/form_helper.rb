module Ransack
  module Helpers
    module FormHelper
      ...
      def sort_link_ransack(search_object, attribute, controller, action, name, *args, &block)
        search, routing_proxy = extract_search_and_routing_proxy(search_object)
        unless Search === search
          raise TypeError, 'First argument must be a Ransack::Search!'
        end
        ...
        s = SortLink.new(search, attribute, args, params, &block)
        options = { q: s.url_options[:q] }

        link_to(url_with_action_and_controller_name(routing_proxy, options, controller, action), s.html_options(args)) do
          sort = search_object.sorts.find {|s| s.name == attribute.to_s}
          content_tag(:span, name.to_s.capitalize, class: 'mr-5') + icon(:fas, sort.present? ? (sort.dir == 'asc' ? 'sort-up' : 'sort-down') : 'sort')
        end
        # link_to(name, url_with_action_and_controller_name(routing_proxy, options, controller, action), s.html_options(args))
      end
      ...
      def url_with_action_and_controller_name(routing_proxy, options_for_url, controller, action)
        if routing_proxy && respond_to?(routing_proxy)
          send(routing_proxy).url_for(controller: controller, action: action, params: options_for_url)
        else
          url_for(controller: controller, action: action, params: options_for_url)
        end
      end
